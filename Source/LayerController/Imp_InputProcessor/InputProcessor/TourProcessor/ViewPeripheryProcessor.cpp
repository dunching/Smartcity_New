...
TourProcessor::FViewPeripheryProcessor::FViewPeripheryProcessor(
	...
	PCPtr->GameplayTasksComponentPtr->StartGameplayTask<UGT_CameraTransformByPawnViewer>(
		 ...
		 )
		 {
			 if (GTPtr)
			 {
				 ...
				 UGameplayStatics::GetAllActorsOfClass(
				                                       GTPtr,
				                                       ALandScapeBase::StaticClass(),
				                                       OutActors
				                                      );

				 for (auto ActorIter : OutActors)
				 {
					 auto LandScapePtr = Cast<
						 ALandScapeBase>(ActorIter);
					 if (LandScapePtr)
					 {
						 if (LandScapePtr->DefaultBuildingCameraSeat.ToSoftObjectPath().IsValid())
						 {
							 GTPtr->
								 ViewerPawnPtr
								 = LandScapePtr->DefaultBuildingCameraSeat.LoadSynchronous();
							 return;
						 }
					 }
				 }
